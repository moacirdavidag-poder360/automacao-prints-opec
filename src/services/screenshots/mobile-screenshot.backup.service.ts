...
`);
    ...
    logger.info(`[INFO] ${iframes.length} iframes de ads encontrados`);

    let targetIframe = null;

    for (const iframe of iframes) {
      try {
        let width = await iframe.getAttribute("width");
        let height = await iframe.getAttribute("height");
        ...
        logger.info(`[INFO] iframe encontrado: ${width}x${height}`);

        if (String(width) === String(320) && String(height) === String(100)) {
          targetIframe = iframe;
          logger.info(`[INFO] 🎯 IFRAME ALVO ENCONTRADO`);
          break;
        }
      } catch (err) {
        logger.warn(`[WARNING] erro ao ler iframe`);
      }
    }
    ...
    if (isMobile) {
      ...
      for (const iframe of iframes) {
        try {
          ...
          const ads = await driver.findElements(By.css(".img_ad"));

          if (ads.length > 0) {
            logger.info(
              `[INFO] Encontrou ${ads.length} .img_ad dentro de iframe`
            );

            for (const ad of ads) {
              const rect = await ad.getRect();
              logger.info(`Ad encontrado: ${rect.width}x${rect.height}`);
            }
          }
